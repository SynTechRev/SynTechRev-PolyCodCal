...
def generate_report(data: List[Dict], config: Dict) -> str:
    ...
    violations_count = sum(
        len(validate_data_compliance(item, requirements)) for item in data
    )
    violations_summary = f"Compliance violations found: {violations_count}"
    report_lines = [
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        usage = "Usage: python legal_data_report.py"
        usage += " <data_path> <config_path> [output_path]"
        print(usage)
        sys.exit(1)
